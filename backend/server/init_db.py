...
def init():
    ...
    headers = {
        'Authorization': "Bearer %s" % requests.post("http://localhost:5000/tokens", auth=HTTPBasicAuth('ralph', 'pwd')).json()["_id"]
    }

    # check if authorization worked
    profile = requests.get("http://localhost:5000/users/ralph", headers=headers)
    print(profile.json())
    ...
    requests.post('http://localhost:5000/posts', json={
        "uri": fake.uri(),
        "category": fake.category()
    }, headers=headers)
    ...
    featured_posts = requests.get("http://localhost:5000/posts", headers=headers).json()
